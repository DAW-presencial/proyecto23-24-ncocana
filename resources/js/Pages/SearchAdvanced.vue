<template>
    ...
    <AuthenticatedLayout>
        ...
        formatDate(resultado.attributes.bookmarkable.release_date) }}</p>
                                <p><strong>{{ $t('Currently at') }}:</strong> {{
        resultado.attributes.bookmarkable.currently_at }}</p>
                            </template>
        ...
        resultado.attributes.bookmarkable.currently_at }}</p>
                            ...
                            <template v-else-if="resultado.attributes.bookmarkable_type === 'App\\Models\\Fanfic'">
                                ...
                                <p><strong>{{ $t('Fandom') }}:</strong> {{ resultado.attributes.bookmarkable.fandom }}
                                </p>
        ...
        resultado.attributes.bookmarkable.read_chapters }}</p>
                                <p><strong>{{ $t('Total Chapters') }}:</strong> {{
        resultado.attributes.bookmarkable.total_chapters }}</p>
                            </template>
                        </div>
                        ...
                        <div v-if="resultado.attributes.tags.length">
                            ...
                            <ul>
                                <li v-for="tag in resultado.attributes.tags" :key="tag.id">{{ $t(tag.name) }}</li>
                            </ul>
                        </div>
                    </Card>
                    <div class="mt-6">
